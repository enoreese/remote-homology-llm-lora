...
def enter(download=False):
    ...
    cols = ['FA-DOMID', 'FA-PDBID', 'SF-DOMID', 'SF-PDBID', 'SF', 'FA']
    ...
    print("Loading DFs...")
    cla = pd.read_csv('/vol/scop/scop-cla-latest.csv', usecols=cols)
    ...
    process.remote(cla, fa_seq, sf_seq)
...
def process_result(pairs_list, cla, fa_seq, sf_seq):
    ...
    filename = f'result_{str(idx)}.csv'
...
def merge_dfs():
    ...
    files = [file for file in os.listdir('/vol/scop/results/') if
             file.endswith('.csv') and file.split('_')[0] == 'result']
    ...
    for file in tqdm(files):
        ...
        non_remote = df[df['remote_homologs'] == False].sample(remote.shape[0] * 3)
    ...
    print("Saving to file...")
    dfs.to_csv('/vol/scop/results/combined_result.csv')
    volume.commit()
...
def calculate_identity():
    ...
    results = pd.read_csv('/vol/scop/results/combined_result.csv', index_col=0)
    ...
    print("Saving to file...")
    results.to_csv('/vol/scop/results/combined_result_threshold.csv')
    volume.commit()
    ...
    blob = bucket.blob(f'llm-bkt/dna_files/scop/combined_result.csv')
    blob.upload_from_filename(filename=f'/vol/scop/results/combined_result.csv', content_type='text/csv')

    blob = bucket.blob(f'llm-bkt/dna_files/scop/combined_result_threshold.csv')
    blob.upload_from_filename(filename=f'/vol/scop/results/combined_result_threshold.csv', content_type='text/csv')
...
def create_dataset():
    ...
    print("Loading df...")
    df = pd.read_csv('/vol/scop/results/combined_result_threshold.csv', index_col=0)
    df.rename({
    ...
    train_path = "/vol/scop/results/soft_rh_train_dataset.json"
    train.to_json(train_path, orient='records', lines=True)
    filename_to_gcp("llm-bkt/dna_files/scop/soft_rh_train_dataset.json", train_path)

    print("Creating test...")
    test_path = "/vol/scop/results/soft_rh_test_dataset.json"
    test.to_json(test_path, orient='records', lines=True)
    filename_to_gcp("llm-bkt/dna_files/scop/soft_rh_test_dataset.json", test_path)
